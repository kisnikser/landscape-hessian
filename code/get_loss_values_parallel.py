...
def process_h_l(h, L, config, device, train_dataset, train_dataloader, val_dataloader):
    ...
    for sample_idx in range(config.num_samples):
        ...
        results_samples.append({"loss_values": loss_values, "accuracy": accuracy_list})

    return {"h": h, "L": L, "val_batch_size": config.val_batch_size, "samples": results_samples}
...
def worker(gpu_id, config_dict, task_queue, result_queue):
    ...
    train_dataset = getattr(datasets, config.dataset_name)("./data", download=True, train=True, transform=transform)
    ...
    val_dataset = getattr(datasets, config.dataset_name)("./data", download=True, train=False, transform=transform)
    val_dataset = torch.utils.data.Subset(val_dataset, np.arange(config.val_size))
    val_dataloader = torch.utils.data.DataLoader(
        val_dataset, batch_size=config.val_batch_size, shuffle=False, num_workers=4, pin_memory=True
    )
...
def main(config):
    ...
    for gpu_id in range(num_gpus):
        p = mp.Process(target=worker, args=(gpu_id, OmegaConf.to_container(config), task_queue, result_queue))
        p.start()
        workers.append(p)
