...
def main(config):
    ...
    train_dataset = name2dataset[config.dataset_name](
        f"~/.pytorch/{config.dataset_name}_data/", download=True, train=True
    )
    train_dataset = torch.utils.data.Subset(train_dataset, np.arange(config.train_size))

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    pipe = pipeline(
        task="image-feature-extraction", model_name="google/vit-base-patch16-224", device=device, pool=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_path", type=str, default="configs/configs_extraction/mnist.yml")
